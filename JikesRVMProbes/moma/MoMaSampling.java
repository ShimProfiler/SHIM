 ...
 */
public class MoMaSampling extends MoMaProfiler {
  ...
  public void init() {
    ...
    for (RVMThread t : RVMThread.threads) {
      if (t != null && t.getName().equals("MainThread")) {
        ...
        sysCall.sysCall.sysThreadBind(Magic.getThreadRegister().pthread_id, 0x8);
  ...
  private void logCounters(long counters[], int tag) {
    for (int i = 0; i <  numberPerfEvent; i++) {
  ...
  public void profile() {
    ...
    logCounters(momaCounters[i++], Magic.getCompiledMethodID(targetThread.framePointer));

    while (true) {
      logCounters(momaCounters[i], Magic.getCompiledMethodID(targetThread.framePointer));
      long distance = momaCounters[i][0] - momaCounters[i-1][0];
